...
# Basic options
parser.add_argument('--content_dir', type=str, default = 'input/',
                    help='File path to the content dir')
parser.add_argument('--content', type=str, default = None, help='filepath to content images')
parser.add_argument('--style_dir', type=str, help='path to style_dir', default='style/')
...
parser.add_argument('--no_gpu', action='store_true', help='specify if no use gpu')
# Advanced options
...
if args.content is not None and args.style is not None:
    print("in single style transfer")
    ...
    content_name = splitext(basename(args.content))[0]
    style_name = splitext(basename(args.style))[0]
    ...
    with torch.no_grad():
        ...
        output_name = '{:s}/{:s}_stylized_{:s}{:s}'.format(
                    args.output, splitext(basename(content_name))[0],
                    splitext(basename(style_name))[0], args.save_ext
                )
        save_image(content, output_name)
    exit(0)

content_files = os.listdir(args.content_dir)
style_files = os.listdir(args.style_dir)

for content_name in content_files:
    for style_name in style_files:


        content = content_tf(Image.open(args.content_dir + content_name))
        style = style_tf(Image.open(args.style_dir + style_name))
